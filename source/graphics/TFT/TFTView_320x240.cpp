 ...
 */
void TFTView_320x240::init(IClientBase *client, AppLibrary *appLib)
{
...
void TFTView_320x240::ui_set_active(lv_obj_t *b, lv_obj_t *p, lv_obj_t *tp)
{
    ...
    if (activePanel) {
        ...
        if (activePanel == objects.messages_panel) {
            ...
            // Stop app when navigating away
            appHost.stopApp(appRunPanel);
        }
    }
...
void TFTView_320x240::apply_hotfix(void)
{
    ...
    // Show apps button when Berry apps are available, otherwise show map button directly
    if (!appHost.getApps().empty()) {
        lv_obj_clear_flag(objects.apps_button, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(objects.map_button, LV_OBJ_FLAG_HIDDEN);
    } else {
        lv_obj_add_flag(objects.apps_button, LV_OBJ_FLAG_HIDDEN);
...
void TFTView_320x240::populateAppsPanel()
{
    ...
    const auto &apps = appHost.getApps();

    // Build combined list: hardcoded Maps + discovered .mapp apps
    struct AppEntry {
        std::string name;
        bool isNativeMap;
        int appIndex; // -1 for native apps
    };

    std::vector<AppEntry> entries;

    // Always add Maps as first app
    {
        AppEntry e;
        e.name = "Maps";
        e.isNativeMap = true;
    ...
    for (int i = 0; i < (int)entries.size(); i++) {
        ...
        // Make cell clickable; encode native map as index -1, Berry apps as their appHost index
        lv_obj_add_flag(cell, LV_OBJ_FLAG_CLICKABLE);
        cell->user_data = reinterpret_cast<void *>(static_cast<intptr_t>(app.appIndex));
        lv_obj_add_event_cb(cell, ui_event_AppItemButton, LV_EVENT_CLICKED, NULL);
    }
}

static void ui_msgbox_approve_cb(lv_event_t *e)
{
...
void TFTView_320x240::ui_event_AppItemButton(lv_event_t *e)
{
    ...
    // Handle native Maps app (index -1)
    if (appIndex < 0) {
    ...
    const auto &apps = THIS->appHost.getApps();
    if (appIndex >= (int)apps.size()) {
        ILOG_ERROR("[Apps] Invalid app index %d (apps size=%u)", appIndex, (unsigned)apps.size());
        return;
    }

    const auto &app = apps[appIndex];
    ILOG_DEBUG("[Apps] Launching app: '%s'", app.name.c_str());

    // Check trust: show approval dialog if not yet verified
    if (!app.trusted || !app.permissionsApproved) {
        // Build permission list string
        std::string permList;
        ...
        std::string body = "Author: " + app.author + "\n\nPermissions:\n" + permList;

        lv_obj_t *msgbox = lv_msgbox_create(lv_screen_active());
        lv_msgbox_add_title(msgbox, app.name.c_str());
        lv_msgbox_add_text(msgbox, body.c_str());
        lv_obj_t *approveBtn = lv_msgbox_add_footer_button(msgbox, _("Approve"));
        lv_obj_t *declineBtn = lv_msgbox_add_footer_button(msgbox, _("Decline"));
        lv_obj_set_style_min_width(msgbox, LV_PCT(70), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_center(msgbox);

        // Store app index on approve button
        approveBtn->user_data = reinterpret_cast<void *>(static_cast<intptr_t>(appIndex));

        // Decline just closes
        lv_obj_add_event_cb(declineBtn, ui_msgbox_approve_cb, LV_EVENT_CLICKED, msgbox);

        // Approve: trust developer, approve permissions, persist, and launch
        // Defer the actual launch via lv_timer so the dialog close renders first
        lv_obj_add_event_cb(
            approveBtn,
            [](lv_event_t *ev) {
                lv_obj_t *approveButton = static_cast<lv_obj_t *>(lv_event_get_target(ev));
                int idx = static_cast<int>(reinterpret_cast<intptr_t>(approveButton->user_data));
                lv_obj_t *mbox = static_cast<lv_obj_t *>(lv_event_get_user_data(ev));
                lv_msgbox_close(mbox);

                const auto &appList = THIS->appHost.getApps();
                if (idx >= 0 && idx < (int)appList.size()) {
                    lv_label_set_text(THIS->appRunLabel, appList[idx].name.c_str());
                    THIS->ui_set_active(objects.apps_button, THIS->appRunPanel, THIS->topAppRunPanel);
                    lv_timer_create(
                        [](lv_timer_t *t) {
                            int appIdx = static_cast<int>(reinterpret_cast<intptr_t>(t->user_data));
                            THIS->appHost.approveAndLaunch(appIdx, THIS->appRunPanel);
                            lv_timer_delete(t);
                        },
                        0, reinterpret_cast<void *>(static_cast<intptr_t>(idx)));
                }
            },
            LV_EVENT_CLICKED, msgbox);

        return;
    }

    // Already approved — defer launch so UI updates first
    lv_label_set_text(THIS->appRunLabel, app.name.c_str());
    THIS->ui_set_active(objects.apps_button, THIS->appRunPanel, THIS->topAppRunPanel);
    lv_timer_create(
        [](lv_timer_t *t) {
            int appIdx = static_cast<int>(reinterpret_cast<intptr_t>(t->user_data));
            THIS->appHost.launchApp(appIdx, THIS->appRunPanel);
            lv_timer_delete(t);
        },
        0, reinterpret_cast<void *>(static_cast<intptr_t>(appIndex)));
}
...
void TFTView_320x240::ui_event_AppCloseButton(lv_event_t *e)
{
    ...
    THIS->appHost.stopApp(THIS->appRunPanel);
...
void TFTView_320x240::task_handler(void)
{
    ...
    // Drain mesh event and state notification queues for running apps
    appHost.tick();
