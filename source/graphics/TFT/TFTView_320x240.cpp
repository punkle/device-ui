...
void TFTView_320x240::init_screens(void)
{
...
#ifdef HAS_APP_FRAMEWORK
    // create apps panel (programmatic, not generated by EEZ Studio)
    {
    ...
    }
    {
        ...
        lv_obj_set_style_align(obj, LV_ALIGN_TOP_LEFT, LV_PART_MAIN | LV_STATE_DEFAULT);
        {
            ...
            lv_label_set_text(label, _("Apps"));
            lv_obj_set_style_text_font(label, &ui_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_pos(label, 25, 0);
            lv_obj_set_style_align(label, LV_ALIGN_LEFT_MID, LV_PART_MAIN | LV_STATE_DEFAULT);
        }
        {
            lv_obj_t *img = lv_image_create(obj);
            lv_obj_set_pos(img, -5, 0);
            lv_obj_set_size(img, 24, 24);
            lv_image_set_pivot(img, 0, 0);
            add_style_top_image_style(img);
            lv_obj_set_style_bg_image_src(img, &img_apps_button_image, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_align(img, LV_ALIGN_LEFT_MID, LV_PART_MAIN | LV_STATE_DEFAULT);
        }
    }
    // create app run panel (where Berry app UI is rendered)
    {
        lv_obj_t *obj = lv_obj_create(objects.main_screen);
        appRunPanel = obj;
        lv_obj_set_pos(obj, LV_PCT(12), LV_PCT(10));
    ...
    }
    {
        ...
        lv_obj_set_style_align(obj, LV_ALIGN_TOP_LEFT, LV_PART_MAIN | LV_STATE_DEFAULT);
        {
            appRunLabel = lv_label_create(obj);
            lv_label_set_text(appRunLabel, "");
            lv_obj_set_style_text_font(appRunLabel, &ui_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_pos(appRunLabel, 25, 0);
            lv_obj_set_style_align(appRunLabel, LV_ALIGN_LEFT_MID, LV_PART_MAIN | LV_STATE_DEFAULT);
        }
        {
            lv_obj_t *closeBtn = lv_button_create(obj);
            lv_obj_set_size(closeBtn, 24, 24);
            lv_obj_set_style_align(closeBtn, LV_ALIGN_RIGHT_MID, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_pos(closeBtn, -5, 0);
            lv_obj_set_style_bg_opa(closeBtn, LV_OPA_50, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_t *closeLabel = lv_label_create(closeBtn);
            lv_label_set_text(closeLabel, LV_SYMBOL_CLOSE);
            lv_obj_center(closeLabel);
            lv_obj_add_event_cb(closeBtn, ui_event_AppCloseButton, LV_EVENT_CLICKED, NULL);
        }
    }
#endif
...
#ifdef HAS_APP_FRAMEWORK
        else if (activePanel == appRunPanel) {
            // Stop Berry app when navigating away
            if (berryRuntime) {
                berryRuntime->stopApp();
                delete berryRuntime;
                berryRuntime = nullptr;
            }
        }
#endif
    }
...
void TFTView_320x240::apply_hotfix(void)
{
    ...
    if (h > 320 && v > 320) {
        lv_obj_t *button[] = {objects.home_button,     objects.nodes_button, objects.groups_button,
                              objects.messages_button, objects.apps_button,  objects.map_button,
...
#ifdef HAS_APP_FRAMEWORK
    lv_obj_clear_flag(objects.apps_button, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(objects.map_button, LV_OBJ_FLAG_HIDDEN);
#endif
...
#ifdef HAS_APP_FRAMEWORK
    lv_obj_set_style_bg_img_recolor_opa(objects.apps_button, opa, LV_PART_MAIN | LV_STATE_DEFAULT);
#else
...
void TFTView_320x240::ui_event_AppsButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code == LV_EVENT_CLICKED && THIS->activeSettings == eNone) {
        THIS->populateAppsPanel();
        THIS->ui_set_active(objects.apps_button, THIS->appsPanel, THIS->topAppsPanel);
    }
}
#endif
...
void TFTView_320x240::loadAppList()
{
    ...
    // Add native apps
    {
        ...
        m.name = "Maps";
        m.isBuiltin = true;
        m.embeddedSource = nullptr;
        m.nativeId = "map";
        appList.push_back(m);
    }

    // Load built-in Berry apps from generated registry
    int count = 0;
    const BuiltinAppEntry *builtins = getBuiltinApps(count);
    for (int i = 0; i < count; i++) {
        AppManifest m;
        m.name = builtins[i].name ? builtins[i].name : "";
        m.version = builtins[i].version ? builtins[i].version : "";
        m.author = builtins[i].author ? builtins[i].author : "";
        m.entry = builtins[i].entry ? builtins[i].entry : "";
        m.embeddedSource = builtins[i].source;
        m.isBuiltin = true;
    ...
    // Scan SD card for user apps in /apps directory
    if (cardDetected) {
        scanSDApps();
    }
}
...
void TFTView_320x240::scanSDApps()
{
    ...
    File appsDir = SDFs.open("/apps");
    if (!appsDir) {
        ILOG_DEBUG("[Apps] No /apps directory on SD card");
        return;
    }
    ...
    do {
        ...
        // On some platforms name() returns full path, extract basename
        size_t slash = dirName.find_last_of("/");
        if (slash != std::string::npos)
            dirName = dirName.substr(slash + 1);
        ...
        std::string appDir = "/apps/" + dirName + "/";
        std::string manifestPath = appDir + "app.json";

        File manifest = SDFs.open(manifestPath.c_str(), FILE_READ);
        if (!manifest) {
            entry.close();
            continue;
        }
        ...
        std::string json;
        json.resize(size);
        manifest.read((uint8_t *)json.data(), size);
        manifest.close();
        entry.close();

        std::string name = jsonStringField(json, "name");
        std::string entryFile = jsonStringField(json, "entry");
        if (name.empty() || entryFile.empty())
            continue;

        AppManifest m;
        m.name = name;
        m.version = jsonStringField(json, "version");
        m.author = jsonStringField(json, "author");
        m.entry = entryFile;
        m.embeddedSource = nullptr;
        m.sdPath = appDir;
        m.isBuiltin = false;
        ...
        if (!icon.empty())
            m.iconPath = appDir + icon;

        ILOG_INFO("[Apps] Found SD app: %s (%s)", m.name.c_str(), appDir.c_str());
        appList.push_back(m);
    } while (true);
    ...
    do {
        ...
        std::string dirName = nameBuf;

        if (!entry.isDirectory() || dirName.empty() || dirName[0] == '.') {
            entry.close();
            continue;
        }
        ...
        FsFile manifest = SDFs.open(manifestPath.c_str(), O_RDONLY);
        if (!manifest) {
            entry.close();
            continue;
        }
...
void TFTView_320x240::populateAppsPanel()
{
    loadAppList();

    // Clear existing children
    lv_obj_clean(appsPanel);
    appIconDescriptors.clear();

    if (appList.empty()) {
        lv_obj_t *label = lv_label_create(appsPanel);
        lv_label_set_text(label, _("No installed apps"));
        lv_obj_set_style_text_font(label, &ui_font_montserrat_16, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_align(label, LV_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
        return;
    }

    // Reserve to ensure pointer stability for lv_image_dsc_t references
    appIconDescriptors.reserve(appList.size());

    for (int i = 0; i < (int)appList.size(); i++) {
        const AppManifest &app = appList[i];

        // Cell container: column flex with icon on top, label below
        lv_obj_t *cell = lv_obj_create(appsPanel);
        lv_obj_set_size(cell, 80, 56);
        ...
        lv_obj_set_style_pad_row(cell, 2, LV_PART_MAIN | LV_STATE_DEFAULT);
        ...
        if (!app.nativeId.empty() && app.nativeId == "map") {
            // Native map app: use existing map icon
            lv_image_set_src(img, &img_map_button_image);
        } else if (app.iconData && app.iconDataSize > 0) {
            // Built-in Berry app with embedded PNG icon
            lv_image_dsc_t dsc = {};
            dsc.header.w = 32;
            dsc.header.h = 32;
            dsc.header.cf = LV_COLOR_FORMAT_RAW;
            dsc.data = app.iconData;
            dsc.data_size = app.iconDataSize;
            appIconDescriptors.push_back(dsc);
            lv_image_set_src(img, &appIconDescriptors.back());
        } else if (!app.iconPath.empty()) {
            // SD card app with filesystem icon path
            lv_image_set_src(img, app.iconPath.c_str());
        } else {
            // Fallback: generic apps icon
            lv_image_set_src(img, &img_apps_button_image);
        }

        // Label below icon
        lv_obj_t *label = lv_label_create(cell);
        lv_label_set_text(label, app.name.c_str());
        lv_obj_set_width(label, 78);
        lv_obj_set_style_text_font(label, &ui_font_montserrat_12, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_text_align(label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_text_color(label, lv_obj_get_style_text_color(appsPanel, LV_PART_MAIN),
                                    LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_label_set_long_mode(label, LV_LABEL_LONG_DOT);

        // Make cell clickable
        lv_obj_add_flag(cell, LV_OBJ_FLAG_CLICKABLE);
        cell->user_data = reinterpret_cast<void *>(static_cast<intptr_t>(i));
        lv_obj_add_event_cb(cell, ui_event_AppItemButton, LV_EVENT_CLICKED, NULL);
    }
}

void TFTView_320x240::ui_event_AppItemButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code != LV_EVENT_CLICKED)
        return;
    ...
    int appIndex = static_cast<int>(reinterpret_cast<intptr_t>(btn->user_data));

    ILOG_DEBUG("[Apps] App button clicked, index=%d", appIndex);
#ifdef ARCH_ESP32
    ILOG_DEBUG("[Apps] Free heap: %u, PSRAM free: %u, largest free block: %u", ESP.getFreeHeap(), ESP.getFreePsram(),
               ESP.getMaxAllocHeap());
#endif

    if (appIndex < 0 || appIndex >= (int)THIS->appList.size()) {
        ILOG_ERROR("[Apps] Invalid app index %d (appList size=%u)", appIndex, (unsigned)THIS->appList.size());
        return;
    }

    const AppManifest &app = THIS->appList[appIndex];
    ILOG_DEBUG("[Apps] Launching app: '%s' (builtin=%d)", app.name.c_str(), app.isBuiltin);

    // Handle native (non-Berry) apps
    if (!app.nativeId.empty()) {
        if (app.nativeId == "map") {
            THIS->ui_set_active(objects.apps_button, objects.map_panel, objects.top_map_panel);
            THIS->loadMap();
    ...
    // Update the app run panel title
    lv_label_set_text(THIS->appRunLabel, app.name.c_str());

    // Switch to app run panel
    ILOG_DEBUG("[Apps] Switching to app run panel");
    THIS->ui_set_active(objects.apps_button, THIS->appRunPanel, THIS->topAppRunPanel);

    // Create Berry runtime on demand and launch the app
    if (!THIS->berryRuntime) {
        ILOG_DEBUG("[Apps] Creating new BerryRuntime");
        THIS->berryRuntime = new BerryRuntime();
        if (!THIS->berryRuntime) {
            ILOG_ERROR("[Apps] Failed to allocate BerryRuntime!");
            return;
        }
        ILOG_DEBUG("[Apps] Initializing BerryRuntime");
        THIS->berryRuntime->init();
    }
    ILOG_DEBUG("[Apps] Calling launchApp");
    bool ok = THIS->berryRuntime->launchApp(app, THIS->appRunPanel);
    ILOG_DEBUG("[Apps] launchApp returned %s", ok ? "true" : "false");
#ifdef ARCH_ESP32
    ILOG_DEBUG("[Apps] Free heap after launch: %u", ESP.getFreeHeap());
#endif
}

void TFTView_320x240::ui_event_AppCloseButton(lv_event_t *e)
{
    lv_event_code_t event_code = lv_event_get_code(e);
    if (event_code != LV_EVENT_CLICKED)
        return;
    ...
    if (THIS->berryRuntime) {
        THIS->berryRuntime->stopApp();
        delete THIS->berryRuntime;
        THIS->berryRuntime = nullptr;
    }

    // Switch back to apps list panel
    THIS->populateAppsPanel();
    THIS->ui_set_active(objects.apps_button, THIS->appsPanel, THIS->topAppsPanel);
}
#endif
